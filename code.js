function pick(node, keys) {
  ...
  function normalizeLayout(node) {
    ...
    if ("layoutMode" in node) {
      ...
      if (Object.keys(info).length) out.autoLayout = cloneJSONSafe(info);
    }
  ...
  function serializeNode(node, opts, depth = 0) {
    const base = {};
    ...
    if ("children" in node && depth < opts.maxDepth) {
      base.children = node.children.map((c) => serializeNode(c, opts, depth + 1));
    }
  ...
  function serializeRoot(root, opts) {
    return serializeNode(root, opts, 0);
  }
  
  function buildVariantExports(componentSet, opts) {
    const exports = [];
  ...
  figma.ui.onmessage = async (msg) => {
    ...
    if (options.exportMode === "page") {
      ...
      if (!sel || sel.length === 0) {
        figma.ui.postMessage({ type: "ERROR", message: "선택된 레이어가 없습니다. 프레임/그룹을 선택하세요." });
        return;
      }
      if (sel.length === 1) root = sel[0];
      else root = { type: "GROUP", id: "selection", name: "selection", visible: true, children: sel };
    }
  
    const meta = buildExportMeta(options);
    ...
    if (options.includeMainTree) {
      exportsList.push({
        ...
        tree: serializeRoot(root, options)
      });
    }
    ...
    if (options.splitVariants) {
      ...
      for (const set of sets) {
        const variants = buildVariantExports(set, options);
    ...
    if (options.exportAssets) {
      try {
        assets = await exportAssetsForRoot(root, options);
      } catch {
        assets = {};
      }
    }
